...
bool LevelSetMethod::extractSurfaceMeshCGAL(const std::string& filename) {
    try {
        if (phi.size() != gridPoints.size() || gridPoints.empty()) {
            throw std::runtime_error("Level set function or grid not initialized for surface extraction.");
        }
 
        // CGAL types for surface meshing
        typedef CGAL::Surface_mesh_default_triangulation_3 Tr; // Triangulation
        typedef CGAL::Surface_mesh_complex_2_in_triangulation_3<Tr> C2t3; // Complex in triangulation
        typedef Tr::Geom_traits GT; // Geometric traits
        typedef GT::Sphere_3 Sphere_3; // Bounding sphere type
        typedef GT::FT FT; // Field type (double)
        
        // Define the implicit function using our class
        LevelSetImplicitFunctionForCGAL implicit_func(gridPoints, phi, GRID_SIZE, GRID_SPACING, gridOrigin);
        
        // Wrap it for CGAL
        // The lambda captures implicit_func by reference. Ensure its lifetime.
        auto cgal_func_wrapper = [&implicit_func](Point_3 p) -> FT {
            return implicit_func(p);
        };
        
        // Bounding sphere for meshing: should encompass the zero level set
        // Origin of sphere can be center of grid, radius large enough.
        Point_3 sphere_center = gridOrigin + Vector_3(BOX_SIZE/2.0, BOX_SIZE/2.0, BOX_SIZE/2.0);
        double sphere_radius_sq = (BOX_SIZE * 0.75) * (BOX_SIZE * 0.75); // Squared radius, bit larger than half box
        Sphere_3 bounding_sphere(sphere_center, sphere_radius_sq);

        // Surface meshing parameters (criteria for mesh quality and density)
        // Angle bound (degrees), radius bound (max triangle circumradius), distance bound (max distance surface to mesh)
        // These values significantly impact mesh quality and generation time.
        double angle_bound = 30.0; // Min angle for triangles
        double radius_bound_rel = GRID_SPACING * 1.5; // Max circumradius relative to grid spacing
        double distance_bound_rel = GRID_SPACING * 0.5; // Max deviation from implicit surface
        CGAL::Surface_mesh_default_criteria_3<Tr> criteria(angle_bound, radius_bound_rel, distance_bound_rel);
        
        // Triangulation and complex
        Tr tr; 
        C2t3 c2t3(tr); 
        
        std::cout << "Starting CGAL surface mesh generation (make_surface_mesh)..." << std::endl;
        CGAL::make_surface_mesh(c2t3, cgal_func_wrapper, criteria, CGAL::Manifold_with_boundary_tag());
        // CGAL::make_surface_mesh(c2t3, cgal_func_wrapper, criteria, CGAL::Non_manifold_tag()); // If non-manifold results are okay
        std::cout << "CGAL make_surface_mesh completed. Converting to Surface_mesh..." << std::endl;
        
        Mesh output_surface_mesh; // CGAL::Surface_mesh<Point_3>
        CGAL::facets_in_complex_2_to_triangle_mesh(c2t3, output_surface_mesh);
        
        if (output_surface_mesh.is_empty()) {
            std::cerr << "Warning: Extracted surface mesh is empty." << std::endl;
        } else {
            std::cout << "Surface mesh has " << output_surface_mesh.number_of_vertices() << " vertices and " 
                      << output_surface_mesh.number_of_faces() << " faces." << std::endl;
        }
        
        // Save the mesh
        if (!CGAL::IO::write_polygon_mesh(filename, output_surface_mesh, CGAL::parameters::stream_precision(17))) {
            throw std::runtime_error("Failed to write surface mesh to file: " + filename);
        }
