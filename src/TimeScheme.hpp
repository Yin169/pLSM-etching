...
class implicitLUSGS : public TimeScheme {
  public:
    implicitLUSGS(double timeStep, double GRID_SPACING = 1.0) : TimeScheme(timeStep, GRID_SPACING) {};
	...
	Eigen::VectorXd GenRHS(const Eigen::VectorXd& phi_m, const Eigen::VectorXd& phi_n, const Eigen::VectorXd& phi_mn) const {
        	return phi_m / tau + (1+gamma)*phi_n / dt + (gamma)*(phi_n - phi_mn) / dt; 
        }
    ...
    Eigen::VectorXd advance(const Eigen::SparseMatrix<double>& A, const Eigen::VectorXd& phi) {
		...
		for (size_t t = 0; t < dualTimeStepping; t++) {
        	Eigen::VectorXd b = GenRHS(phi_m, phi_n, phi_mn);
            ...
            if (t % 10 == 0){
                std::cout << "Iteration: " << t << ", L2 norm: " << l2_norm << std::endl;
            }
			if (l2_norm < 1e-10) {
